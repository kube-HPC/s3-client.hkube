...
class S3Client {
    ...
    async put(options) {
        const params = {
            Bucket: this._parseBucket(options.Bucket),
            Key: this._parseKey(options.Key),
    ...
    _parseBucket(bucketName) {
        if (bucketName == null) {
            throw new ReferenceError('Bucket cannot be null');
        }
        if (typeof bucketName !== 'string') {
            throw new TypeError('Bucket must be a string');
        }
        if (bucketName.length === 0 || !bucketName.trim()) {
    ...
    _parseKey(key) {
        if (key == null) {
            throw new ReferenceError('Key cannot be null');
        }
