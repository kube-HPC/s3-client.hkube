...
class S3Client {
    ...
    async _createBucketIfNotExists(bucket) {
        const exists = await this._isBucketExists(bucket);
        if (!exists) {
            await this._s3.createBucket(bucket).promise();
        }
        return bucket;
    }

    async _isBucketExists(bucket) {
        try {
            await this._s3.headBucket({ Bucket: bucket.Bucket }).promise();
            return true;
        }
        catch (error) {
